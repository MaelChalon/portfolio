...
if(!isset($_SESSION['ActiviteData'])) {
...
if (isset($_GET["idActivite"])) {
    $_SESSION['ActiviteData']['isModifyActity'] = true;
    $_SESSION['ActiviteData']['idActiviteToModify'] = $_GET["idActivite"];
}
...
<head>
    ...
    <?php
    if (!$_SESSION['ActiviteData']['isModifyActity']) {
...
<body class="scroll_div_element scrollbar_style_1">
    ...
    <main id="page_educateur_nouvelle_activite" class="flex_column flex_double_center center">
        ...
        <div class="fill flex_column flex_double_center" id="nouvelle_activite">
            <form method="post" class="block_style_1 flex_column flex_double_center form_style_1">
                <?php
                if (empty($_SESSION['ActiviteData']['isModifyActity'])) {
                ...
                <div class="flex_row space_around fill_width flex_wrap"><!-- id="section_gestion_competence" -->
                    ...
                    <div class="flex_row">
                        <a id="bouton_nouvelle_competence" href="#/" class="button_style_2" onclick='saveActiviteData(<?php echo $_SESSION["ActiviteData"]["isModifyActity"] ? "true" : "false"; ?>, <?= $_SESSION["ActiviteData"]["idActiviteToModify"] ?>);'>Créer une nouvelle compétence</a>
                    </div>
                </div>

                <input type="hidden" id="modify_activite" name="isModifyActivite" value="<?php if ($_SESSION['ActiviteData']['isModifyActity']) echo "1";
                                                                                            else echo "0"; ?>">
                ...
                <input type="hidden" id="send_ajout_activite" name="isAjoutActivite" value="<?= $_SESSION['ActiviteData']['isModifyActity'] ? '0' : '1' ?>">
                <input type="hidden" id="idIME" name="idIME" value="<?= $current_IME ?>">
                <input type="hidden" id="return_page" name="return_page" value="<?php if($_SESSION['ActiviteData']['isModifyActity']) echo "/educateur/activites"; else echo "/educateur/nouvelle_activite" ?>">
                ...
                <button id="creer_activite_button" type="submit" class="button_style_0"><?php if (empty($_SESSION['ActiviteData']['isModifyActity'])) {
    ...
    ?>
            <script>
                addCompetence(<?= json_encode($comp) ?>)
            </script>
    <?php
        endforeach;
    }
